...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: ZuMY.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: ZuMYd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeZuMYamount=<n> " + _("Keep N ZuMY anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ZuMY is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ZuMY is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("ZuMY version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of ZuMY") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart ZuMY to complete") << "\n";
    ...
    nAnonymizeZuMYAmount = GetArg("-anonymizeZuMYamount", 0);
    if(nAnonymizeZuMYAmount > 999999) nAnonymizeZuMYAmount = 999999;
    if(nAnonymizeZuMYAmount < 2) nAnonymizeZuMYAmount = 2;
    ...
    LogPrintf("Anonymize ZuMY Amount %d\n", nAnonymizeZuMYAmount);
