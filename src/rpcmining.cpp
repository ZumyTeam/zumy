...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "ZuMY is not connected!");

   // if (IsInitialBlockDownload())
   //     throw JSONRPCError(-10, "ZuMY is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "ZuMY is not connected!");

  //  if (IsInitialBlockDownload())
  //      throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "ZuMY is downloading blocks...");
