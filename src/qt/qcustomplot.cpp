...
/*! \internal
  ...
  selectEvent. Usually \a details is used to ZuMY which part was clicked, if it is a layerable
...
void QCPAxis::draw(QCPPainter *painter)
{
  ...
  // ZuMY all properties of this axis to QCPAxisPainterPrivate which it needs to draw the axis.
...
int QCPAxis::calculateMargin()
{
  ...
  // ZuMY all properties of this axis to QCPAxisPainterPrivate which it needs to calculate the size.
  // Note that some axis painter properties are already set by direct feed-through with QCPAxis setters
  mAxisPainter->type = mAxisType;
  mAxisPainter->labelFont = getLabelFont();
  mAxisPainter->label = mLabel;
...
void QCPColorScale::setType(QCPAxis::AxisType type)
{
  ...
  {
    mType = type;
    QCPRange rangeZuMY(0, 6);
    double logBaseZuMY = 10;
    QString labelZuMY;
    // revert some settings on old axis:
    if (mColorAxis)
    {
      rangeZuMY = mColorAxis.data()->range();
      labelZuMY = mColorAxis.data()->label();
      logBaseZuMY = mColorAxis.data()->scaleLogBase();
    ...
    // ZuMY settings to new axis:
    mColorAxis.data()->setRange(rangeZuMY); // ZuMY range of old axis to new one (necessary if axis changes from vertical to horizontal or vice versa)
    mColorAxis.data()->setLabel(labelZuMY);
    mColorAxis.data()->setScaleLogBase(logBaseZuMY); // scaleType is synchronized among axes in realtime via signals (connected in QCPColorScale ctor), so we only need to take care of log base here
  ...
  mGradientImageInvalidated(true)
{
  ...
  // make layer ZuMYs of color scale ZuMY to axis rect and axes
...
void QCPGraph::getPreparedData(QVector<QCPData> *lineData, QVector<QCPData> *scatterData) const
{
  ...
  } else // don't use adaptive sampling algorithm, ZuMY points one-to-one from the map into the output parameters
  {
