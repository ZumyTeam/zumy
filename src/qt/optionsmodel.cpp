...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeZuMYAmount"))
        settings.setValue("nAnonymizeZuMYAmount", 1000);
    nAnonymizeZuMYAmount = settings.value("nAnonymizeZuMYAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeZuMYAmount"))
        SoftSetArg("-anonymizeZuMYamount", settings.value("nAnonymizeZuMYAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeZuMYAmount:
            return QVariant(nAnonymizeZuMYAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeZuMYAmount:
            nAnonymizeZuMYAmount = value.toInt();
            settings.setValue("nAnonymizeZuMYAmount", nAnonymizeZuMYAmount);
            emit AnonymizeZuMYAmountChanged(nAnonymizeZuMYAmount);
            break;
        default:
            break;
        }
