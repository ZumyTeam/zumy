...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeZuMYAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeZuMYAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeZuMYAmount = strAnonymizeZuMYAmount.remove(strAnonymizeZuMYAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeZuMYAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeZuMYAmount*COIN) nMaxToAnonymize = nAnonymizeZuMYAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeZuMYAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeZuMYAmount));
        strAnonymizeZuMYAmount = strAnonymizeZuMYAmount.remove(strAnonymizeZuMYAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeZuMYAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
