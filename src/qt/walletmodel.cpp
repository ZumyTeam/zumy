...
bool WalletModel::validateAddress(const QString &address)
{
    ...
    CZuMYAddress addressParsed(sAddr);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CScript scriptPubKey = GetScriptForDestination(CZuMYAddress(rcp.address.toStdString()).Get());
...

    {
        ...
        // reject zumy fee
        unsigned int zumyfee = (transaction.getTransactionSize() == 0 ? nAddresses : transaction.getTransactionSize());
        zumyfee = ((MIN_RELAY_TX_FEE * zumyfee) * 10000);
        if (nFeeRequired > zumyfee){
            LogPrintf("nFeeRequired: %d -- zumyFee: %d\n", nFeeRequired, zumyfee);
            return zumyFee;
        }
    }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            CScript scriptPubKey;
            scriptPubKey.SetDestination(CZuMYAddress(sAddr).Get());
            vecSend.push_back(make_pair(scriptPubKey, rcp.amount));
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CTxDestination dest = CZuMYAddress(strAddress).Get();
    ...
    const CTxDestination &address, const std::string &label, bool isMine, ChangeType status)
{
    ...
    {
    QString strAddress = QString::fromStdString(CZuMYAddress(address).ToString());
...
{
    // ZuMY context; old object no longer relocks wallet
    *this = rhs;
    rhs.relock = false;
}
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CZuMYAddress(address).ToString())].push_back(out);
    }
}
