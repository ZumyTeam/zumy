...
const string strMessageMagic = "ZuMY Signed Message:\n";
...
bool AcceptToMemoryPool(CTxMemPool& pool, CTransaction &tx, bool fLimitFree,
                        bool* pfMissingInputs, bool fRejectzumyFee, bool ignoreFees)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
...

    {
        ...
        if (fRejectzumyFee && nFees > MIN_RELAY_TX_FEE * 10000)
            return error("AcceptableInputs: : zumy fees %s, %d > %d",
                         hash.ToString(),
...
bool AcceptableInputs(CTxMemPool& pool, const CTransaction &txo, bool fLimitFree,
                         bool* pfMissingInputs, bool fRejectzumyFee, bool isDSTX)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
        *pfMissingInputs = false;
...

    {
        ...
        if (fRejectzumyFee && nFees > txMinFee * 10000)
            return error("AcceptableInputs: : zumy fees %s, %d > %d",
                         hash.ToString(),
                         nFees, MIN_RELAY_TX_FEE * 10000);
...
bool CMerkleTx::AcceptToMemoryPool(bool fLimitFree, bool fRejectzumyFee, bool ignoreFees)
{
    return ::AcceptToMemoryPool(mempool, *this, fLimitFree, NULL, fRejectzumyFee, ignoreFees);
}
        ...
        BOOST_FOREACH(CAddress& addr, vAddr)
        {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            {
                // Relay to a limited number of other nodes
                {
                    ...
                    hashRand = Hashskein512(BEGIN(hashRand), END(hashRand));
                    ...
                    BOOST_FOREACH(CNode* pnode, vNodes)
                    {
                        ...
                        hashKey = Hashskein512(BEGIN(hashKey), END(hashKey));
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        uint256 hash = Hashskein512(vRecv.begin(), vRecv.begin() + nMessageSize);
