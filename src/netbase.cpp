...
#include "hash.h"
#include "hashalgo/skein/hashblock.h"
#include "sync.h"
#include "uint256.h"
#include "util.h"
...
static const unsigned char pchZumyCat[] = {0xFD,0x87,0xD8,0x7E,0xEB,0x43};
static const unsigned char pchGarliCat[] = {0xFD,0x60,0xDB,0x4D,0xDD,0xB5};

bool CNetAddr::SetSpecial(const std::string &strName)
{
    if (strName.size()>6 && strName.substr(strName.size() - 6, 6) == ".Zumy") {
        std::vector<unsigned char> vchAddr = DecodeBase32(strName.substr(0, strName.size() - 6).c_str());
        if (vchAddr.size() != 16-sizeof(pchZumyCat))
            return false;
        memcpy(ip, pchZumyCat, sizeof(pchZumyCat));
        for (unsigned int i=0; i<16-sizeof(pchZumyCat); i++)
            ip[i + sizeof(pchZumyCat)] = vchAddr[i];
        return true;
    }
    if (strName.size()>11 && strName.substr(strName.size() - 11, 11) == ".oc.b32.i2p") {
        ...
        if (vchAddr.size() != 16-sizeof(pchGarliCat))
            return false;
        memcpy(ip, pchZumyCat, sizeof(pchGarliCat));
...
{
    return (memcmp(ip, pchZumyCat, sizeof(pchZumyCat)) == 0);
}
...
{
    if (IsTor())
        return EncodeBase32(&ip[6], 10) + ".Zumy";
    if (IsI2P())
...
{
    uint256 hash = Hashskein512(&ip[0], &ip[16]);
    uint64_t nRet;
